...
import z from "zod";
import { animeSchema, animeStatusSchema, type AnimeStatus } from "../common/anime.model";
...
export const animeListStatusIdToAnimeListStatus = z.codec(z.number(), animeStatusSchema, {
	encode: (status) => animeListItemStatusIdMap[status],
	decode(statusId) {
		const statusStatusIdEntries = Object.entries(animeListItemStatusIdMap) as Entries<
			typeof animeListItemStatusIdMap
		>;
...
export const animeListItemDtoToAnime = z.codec(animeListItemDtoSchema, animeSchema, {
	encode: (value) => ({
		anime_id: value.id,
